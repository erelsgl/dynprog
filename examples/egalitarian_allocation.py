...
def egalitarian_partition(valuation_matrix):
    """
    Returns the max-min value - does *not* return the partition itself.

    >>> egalitarian_partition([[11,22,33,44],[11,22,33,44]])
    (55, [[0, 3], [1, 2]])
    >>> egalitarian_partition([[11,22,33,44],[44,33,22,11]])
    (77, [[2, 3], [0, 1]])
    >>> egalitarian_partition([[11,22,33,44,55],[44,33,22,11,55],[55,66,77,88,99]])
    (77, [[2, 3], [0, 1], [4]])
    """
    # Algorithm: add the numbers one by one to all possible bundles.
    num_of_agents   = len(valuation_matrix)
